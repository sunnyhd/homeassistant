    ...
    for bri in range(1, 256, 10):
        for hue in range(0, 361, 10):
            for sat in range(0, 101, 10):
                ...
                light.turn_on(entity_id=entity_id, brightness=bri, hs_color=[hue, sat])
                # 2.Wait 30 Seconds to get correct power values
                task.sleep(delay_time_secs)
                # 3.Read power values
                w = float(state.get(powermeter))
                log.info(f"The power consumption for brightness {bri}, hue {hue} and saturation {sat} is {w} watt")
                ...
                try:
                    ...
                    record_to_insert = (manufacturer, bulb, bri, None, None, None, None, w, hue, sat)
                    cursor.execute(postgres_insert_query, record_to_insert)

                    connection.commit()
                    count = cursor.rowcount
                    log.info(f"{count} Record(s) inserted successfully into bulb_powermeasures table")

                except (Exception, psycopg2.Error) as error:
                    log.info(f"Failed to insert record into bulb_powermeasures table: {error}")
